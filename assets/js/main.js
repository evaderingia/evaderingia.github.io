...
function createMeteor() {
    ...
    meteor.innerHTML = `<svg viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
      <polygon points="18,2 22,13 34,13 24,20 28,32 18,25 8,32 12,20 2,13 14,13" fill="#ffe600" stroke="#ffd700" stroke-width="2"/>
      <polygon points="18,6 20,12 26,12 21,16 23,22 18,18 13,22 15,16 10,12 16,12" fill="#fffde0" opacity="0.5"/>
    </svg>`;
    const scale = 0.6 + Math.random() * 1.4;
    meteor.style.transform = `scale(${scale})`;
    const startTop = Math.random() * window.innerHeight * 0.33;
    meteor.style.top = `${startTop}px`;
    meteor.style.right = `${Math.random() * 60}px`;
    meteor.style.left = 'auto';
    meteor.style.animationDuration = `${1.6 + Math.random() * 1.2}s`;
    const xRange = 60 + Math.random() * 30;
    const yRange = 70 + Math.random() * 25;
    meteor.style.setProperty('--xRange', `-${xRange}vw`);
    meteor.style.setProperty('--yRange', `${yRange}vh`);
